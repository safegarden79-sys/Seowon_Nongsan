...
- **낙찰 줄의 id 는 `시장 · 날짜 · 번호 · 수량 · 단가`로 만듭니다**(`lotKey`). 같은 낙찰을
  다시 읽어도 같은 id 가 나와야 세어둔 박스 수와 주석이 유지됩니다.
  품목·생산자·등급은 **일부러 뺐습니다** — 인식이 자주 틀리기 때문입니다
  (실측: 같은 사람이 `박지혜`와 `박지`로 읽혔고, 품목이 통째로 비기도 했습니다).
  수량과 단가는 거래금액 ÷ 단가 검산을 통과한 값이라 가장 믿을 만합니다.
  실제 19줄로 확인했을 때 이 조합만으로 모두 구분됩니다.
  **id 생성 규칙을 바꾸면 이미 목록에 있는 줄과 새로 읽은 줄이 서로 다른 것이 되어
  중복으로 쌓입니다.** 바꿔야 한다면 목록이 빈 상태에서 하세요.
- **사진에서 읽은 줄은 검수 화면 없이 바로 목록에 들어갑니다.** 다시 올리면 이미 있는
  id 는 건너뛰고 새 줄만 더합니다. 기존 줄은 손대지 않으므로 챙긴 박스 수·주석이 남습니다.
  품목을 못 읽은 줄은 목록에 노란 `품목?` 로 표시됩니다.
